...
def update_seed():
        ...
        return new_seed
def check_inputs(prompt):
    return gr.Button(interactive=True) if prompt is not None else gr.Button(interactive=False)
def generate_image(seed_input, width_input, height_input, prompt_text, lora_dropdown):
    error_image_path = os.path.join(ERROR, "error.jpg")
    try:
        with open("Text2Image.json", "r", encoding="utf-8") as file_json:
        ...
        prompt["31"]["inputs"]["file_path"] = OUTPUT_DIR
...
with gr.Blocks(title="Tạo hình ảnh gợi ý phòng, nội thất", theme='ParityError/Interstellar', css=css) as demo:
     gr.Markdown("# AI gợi ý tạo hình ảnh phòng, nội thất", elem_classes="markdown-spacing")
     ...
     with gr.Row():
        with gr.Column():
            ...
            prompt_text = gr.Textbox(label="Mô tả của bạn", placeholder="Phòng đơn, một giường ngủ...", lines=5)
     ...
     submit_btn.click(
        fn=generate_image,
        inputs=[seed_number, width_slider, height_slider, prompt_text, lora_dropdown],
